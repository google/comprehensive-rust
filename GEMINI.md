...
rust_binary(
    ...
    target_compatible_with = [
        "@platforms//os:none",
        "@platforms//cpu:aarch64",  # or @platforms//cpu:armv7e-mf
    ],
)
```
...
### Host-Executed QEMU Tests & Platform Transitions

By default, Bazel test targets run on the host system. Since the
target binaries require cross-compiling, we use Starlark
configuration transitions in `platforms/transition.bzl` (via rules
like `aarch64_binary`) to transition the target binary's compilation
mode to the target platform while letting the wrapper `sh_test`
execute on the host:
...
# In target BUILD.bazel (or wrapped in macros like aps_example):
aarch64_binary(
